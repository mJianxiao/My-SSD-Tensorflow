...
# 导入新训练的模型参数
ckpt_filename = '../train_model/model.ckpt-90387'  # 注意xxx代表的数字是否和文件夹下的一致
...
# 主流程函数
def process_image(img, select_threshold=0.39, nms_threshold=.01, net_shape=(512, 512)):
...
for i in range(rclasses.shape[0]):
    classname = ['non-motor', 'car', 'person']
    cls_label.append(classname[rclasses[i]-1])
cls_name = np.array(cls_label)

# newrclasses = np.hstack((rclasses1, rclasses2))
# newrscores = np.hstack((newrscores1, newrscores2))
# newrbboxes = np.concatenate((rbboxes1 , rbboxes2), axis=0)


#clslabel = ['nonmontor', 'nonmontor', 'car', 'nonmontor', 'nonmontor', 'car', 'nonmontor', 'nonmontor', 'nonmontor']
print(np.array(cls_label))


# visualization.bboxes_draw_on_img(img, rclasses, rscores, rbboxes, visualization.colors_plasma)
visualization.plt_bboxes(img, cls_name, rscores, rbboxes)
'''
